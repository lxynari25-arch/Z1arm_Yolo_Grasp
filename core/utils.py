...
def save_debug_images(rgb_img, depth_vis_img, depth_frame):
    ...
    Args:
        rgb_img: RGB彩色图
        depth_vis_img: 可视化的深度图
        depth_frame: 原始深度数据
    """
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S_%f")[:-3]  # 添加毫秒级时间戳
    depth_save_path = f'{ROOT}/../output/imgSave/debug/depth_visualization_{timestamp}.png'
    rgb_save_path = f'{ROOT}/../output/imgSave/debug/rgb_image_{timestamp}.png'
    raw_depth_save_path = f'{ROOT}/../output/imgSave/debug/raw_depth_{timestamp}.npy'
    
    os.makedirs(os.path.dirname(depth_save_path), exist_ok=True)
    
    # 保存可视化深度图
    cv2.imwrite(depth_save_path, depth_vis_img)
    # 保存原始深度数据以供后续验证
    np.save(raw_depth_save_path, depth_frame)
    # 保存RGB彩色图
    cv2.imwrite(rgb_save_path, cv2.cvtColor(rgb_img, cv2.COLOR_RGB2BGR))
